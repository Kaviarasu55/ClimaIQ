...
function updateCityTags() {
  ...
  const ids = ["plannerCityTag", "chatCityTag", "vibeCityTag", "packingCityTag"];
  ids.forEach(id => {
...
async function fetchWeather(city) {
  try {
    ...
    document.getElementById("errorText").textContent = "Can't connect to server.";
    document.getElementById("errorMsg").classList.remove("hidden");
  }
}
...
async function fetchAI(weatherData, profession) {
  document.getElementById("aiText").textContent = "Analyzing weather for your profession...";
...
Cloud Cover: ${weatherData.clouds}%
...
`;
  ...
  try {
    ...
    document.getElementById("aiText").textContent = "AI insight unavailable right now.";
  }
}

// ─── Activity Planner ─────────────────────────────────────
document.getElementById("plannerBtn").addEventListener("click", () => runPlanner());
...
async function runPlanner() {
  ...
  if (!city) { alert("Please search a city on Dashboard first!"); return; }
  if (!activity) return;

  document.getElementById("plannerResult").classList.remove("hidden");
  document.getElementById("plannerText").textContent = "Analyzing conditions for your activity...";
  document.getElementById("confidenceScore").textContent = "...";

  let forecastData = [];
  try {
  const forecastRes = await fetch(`${BASE_URL}/forecast?city=${encodeURIComponent(city)}`);
  forecastData = await forecastRes.json();
  } catch {
  // falls through to AI with empty forecast, calculateConfidence handles fallback
  }
...
`;
  ...
  try {
    ...
    const activityType = activityTypeMatch ? activityTypeMatch[1].trim() : "outdoor_physical";
    const aiScore = confidenceMatch ? parseInt(confidenceMatch[1]) : null;
    const score = aiScore !== null ? aiScore : calculateConfidence(currentWeatherData, activityType);
    ...
    document.getElementById("plannerText").textContent = `Best Time: ${bestTime}\n\n${tips}`;

  } catch (err) {
    document.getElementById("plannerText").textContent = "Could not analyze activity right now.";
  }
}
// ─── Packing List ─────────────────────────────────────────
document.getElementById("packingBtn").addEventListener("click", async () => {
  const destination = document.getElementById("packingDestination").value.trim() || currentCity;
  ...
  document.getElementById("packingText").textContent = "Generating packing list...";
...
Current Weather: ${currentWeatherData ? `${currentWeatherData.temp}°C, ${currentWeatherData.condition}, humidity ${currentWeatherData.humidity}%` : "unknown"}
...
`;
  ...
  try {
    ...
    document.getElementById("packingText").textContent = "Could not generate packing list right now.";
  }
});
...
async function sendChat() {
  ...
  const weatherContext = currentWeatherData
    ? `Current weather in ${currentWeatherData.city}: ${currentWeatherData.temp}°C, ${currentWeatherData.condition}, humidity ${currentWeatherData.humidity}%, wind ${currentWeatherData.wind} m/s`
...
async function fetchVibe() {
  ...
  if (d.temp > 38) { mood = "🥵 Scorching"; moodDesc = "Extreme heat — stay hydrated and avoid direct sun."; }
  else if (d.temp > 32) { mood = "😓 Hot & Humid"; moodDesc = "Feels heavy outside. Light clothing recommended."; }
  else if (d.temp > 25) { mood = "😊 Warm & Nice"; moodDesc = "Great weather for outdoor activities!"; }
  else if (d.temp > 18) { mood = "🌤 Cool & Fresh"; moodDesc = "Pleasant temperature. Perfect for a walk."; }
  else if (d.temp > 10) { mood = "🧥 Chilly"; moodDesc = "Layer up before heading out."; }
  else { mood = "🥶 Cold"; moodDesc = "Bundle up well today!"; }

  if (d.condition.includes("rain")) { mood = "🌧 Rainy Vibes"; moodDesc = "Cozy indoor weather. Carry an umbrella if going out."; }
  if (d.condition.includes("storm")) { mood = "⛈ Stormy"; moodDesc = "Stay indoors if possible. Severe weather alert."; }
...
Temperature: ${d.temp}°C, Feels Like: ${d.feels_like}°C
...
`;
  ...
  try {
    ...
    document.getElementById("clothingText").textContent =
      clothingMatch ? clothingMatch[1].trim() : text;
    document.getElementById("summaryText").textContent =
      summaryMatch ? summaryMatch[1].trim() : "";

  } catch (err) {
    document.getElementById("clothingText").textContent = "Unavailable right now.";
